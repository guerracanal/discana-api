...
def get_my_albums():
    ...
    while True:
        ...
        for item in results['items']:
            ...
            #album['genre_type'] = obtener_generos(artist_name=item['artists'][0]['name'], album_name=item['name'])
            #album['genre_type'] = item['genres']
            #print(album['genre_type'])
            albums.append(album)
        offset += limit
    return albums

def get_albums(artist,country):
    sp = get_spotify()
    ...
    while True:
        try:
            ...
            results = sp.artist_albums(artist_id, album_type='album,single,compilation', country=country, limit=limit, offset=offset)
            print(results['href'])
    ...
    for item in items:
        try:
            ...
            #album['genre_type'] = obtener_generos(artist_name=item['artists'][0]['name'], album_name=item['name'])
            #album['genre_type'] = album_info['genres']
            #print(album['genre_type'])
            ...
            print(f"No se pudo obtener información adicional para el álbum {album_name}: {e}")
            return (artist_name, albums)
...
def get_artist_albums(artist_name):
    ...
    albums = []
    results = sp.artist_albums(artist_id, album_type='album,single,compilation', country='US')
    print(results['href'])
...
def custom_data(image, title, subtitle):
    data = {}
    data.update({'album_name': title})
    data.update({'album_artist': subtitle})
    #data.update({'record' : r['label']})
    data.update({'playtime' : ''})
    data.update({'tracks' : ''})
    data.update({'album_art': image})
    data.update({'album_type': ''})
    data.update({'total_tracks': ''})
    data.update({'label_tracks': ''})
    data.update({'type': 'custom'})
    #data.update({'copyright': r['copyrights'][0]['text']})
    data.update({'release_date' : ''})
    #data.update({'popularity' : get_popularity_level(r['popularity']) })
...
def dominant_colors(image):
    ...
    # Asignar códigos y contar ocurrencias
    vecs, _dist = vq(ar, codes)
...
def obtener_generos(artist_name, album_name):
    # Formatear los nombres para la URL
    artist_name = artist_name.replace(' ', '_')
    album_name = album_name.replace(' ', '_')
    
    # URL de búsqueda en Rate Your Music
    url = f"https://rateyourmusic.com/release/album/{artist_name}/{album_name}/"

    try:
        # Hacer la solicitud web
        response = requests.get(url)
        response.raise_for_status()  # Si hay un error en la solicitud, levantar una excepción
        soup = BeautifulSoup(response.text, 'html.parser')

        # Encontrar los géneros en la página
        genre_tags = soup.select('.release-genres-list a')
        genres = [tag.get_text() for tag in genre_tags]
